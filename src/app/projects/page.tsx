...
// --- CUSTOM SVG ICONS FOR SMART DETECTOR ---
...
const ProjectPage = () => {
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          ...
          items.push({
            ...
            subCategory: data.subCategory, // Ensure this matches Admin Panel case
  ...
  // --- SMART URL BUTTON RENDERER ---
  const renderSmartButtons = (proj: any) => {
    const detectedButtons = (proj.smartLinks || []).filter((u: string) => u.trim() !== "").map((url: string) => {
      const lowerUrl = url.toLowerCase();
      if (lowerUrl.includes('behance.net')) return { url, label: "Visit Behance", color: "bg-[#1769ff] text-white", icon: <BehanceIcon /> };
      if (lowerUrl.includes('github.com')) return { url, label: "Visit GitHub", color: "bg-black text-white", icon: <Github className="w-4 h-4" /> };
      if (lowerUrl.includes('youtube.com') || lowerUrl.includes('youtu.be')) return { url, label: "Watch on YouTube", color: "bg-[#FF0000] text-white", icon: <Youtube className="w-4 h-4" /> };
      if (lowerUrl.includes('artstation.com')) return { url, label: "Visit ArtStation", color: "bg-[#13aff0] text-white", icon: <ArtStationIcon /> };
      if (lowerUrl.includes('pinterest.com')) return { url, label: "Visit Pinterest", color: "bg-[#E60023] text-white", icon: <PinterestIcon /> };
      if (lowerUrl.includes('linkedin.com')) return { url, label: "Visit LinkedIn", color: "bg-[#0A66C2] text-white", icon: <Linkedin className="w-4 h-4" /> };
      if (lowerUrl.includes('instagram.com')) return { url, label: "Visit Instagram", color: "bg-gradient-to-r from-[#833AB4] via-[#FD1D1D] to-[#FCB045] text-white", icon: <Instagram className="w-4 h-4" /> };
      if (lowerUrl.includes('tiktok.com')) return { url, label: "Visit TikTok", color: "bg-black text-white", icon: <TikTokIcon /> };
    ...
    return (
      <div className="flex flex-wrap gap-3 mt-4 md:mt-0 items-center justify-start md:justify-end">
        {proj.link && (
          <Link href={proj.link} target="_blank" className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold transition-all hover:scale-105 flex items-center gap-2 shrink-0 shadow-lg shadow-blue-500/20">
            ...
            >
              ...
              <div className="overflow-y-auto flex-1 hide-scrollbar">
                <AnimatePresence mode="wait">
                  ...
                  /* STEP 3: PROJECT GRID (Inside Subcategory) */
                  : selectedSubcategory ? (
...
export default ProjectPage;"use client";
import { useState, useEffect } from "react";
import { collection, getDocs, query, orderBy } from "firebase/firestore";
import { db } from "@/lib/firebase"; 
import FramerWrapper from "@/components/animation/FramerWrapper";
