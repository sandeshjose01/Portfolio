        ...
        {categories.map((cat, index) => (
          <FramerWrapper key={cat.id} y={20} delay={index * 0.1}>
            <motion.div 
              ...
              className="group relative cursor-pointer aspect-[16/10] w-full bg-white/10 rounded-2xl overflow-hidden border border-white/40 shadow-sm"
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 backdrop-blur-xl bg-white/30 border-t border-white/50">
      ...
      <AnimatePresence>
        {selectedCategory && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 md:p-6">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={handleCloseAll} className="absolute inset-0 bg-white/40 backdrop-blur-md" />
            ...
            >
              ...
              <div className="flex justify-between items-center p-6 sticky top-0 z-50 bg-white/40 backdrop-blur-xl border-b border-white/40">
                ...
                <button onClick={handleCloseAll} className="p-2 rounded-xl bg-white/40 hover:bg-red-500/10 text-slate-800 border border-white/60 transition-all"><X className="w-6 h-6" /></button>
              </div>
                    ...
                    <motion.div key="subs" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-8 md:p-12 space-y-10">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {selectedCategory.subcategories.map((sub: string) => (
                          <button key={sub} onClick={() => setSelectedSubcategory(sub)} className="flex items-center justify-between p-8 bg-white/40 border border-white hover:bg-white/60 hover:border-blue-300 transition-all group text-left rounded-2xl shadow-sm">
