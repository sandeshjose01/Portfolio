...
// // text: Behance is NOT in this list because it doesn't exist in Lucide
import { Facebook, Linkedin, Globe, Twitter, ExternalLink } from "lucide-react";
...
export default function SocialLinks({ socials }: any) {
  ...
  return (
    <div className="flex gap-3">
      ...
      {/* // text: Using Twitter icon for Behance to prevent the crash */}
      {socials?.twiter && <Link href={socials.behance} target="_blank" className={iconStyle}><Twitter size={18}/></Link>}
      ...
      {/* // text: Fallback if Admin is completely empty */}
      {!socials && (
         <Link href="#" className={iconStyle}><ExternalLink size={18}/></Link>
      )}
    </div>
  );
}
